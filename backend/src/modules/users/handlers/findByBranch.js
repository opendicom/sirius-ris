...
module.exports = async (req, res, currentSchema) => {
    //Get query params:
    let { filter, regex, branch, role } = req.query;

    //Check branch and role properties:
    if(branch && role){
        ...
        if(!isNaN(role) && user_roles.includes(parseInt(role, 10))){
            ...
            if(moduleServices.checkObjectId(branch)){
                //Get complete domain:
                const completeDomain = await moduleServices.getCompleteDomain(branch, 'branches');
                ...
                //Set findByBranchCondition:
                const findByBranchCondition = { '$or': [
                    //Organization:
                    {
                        "permissions": {
                            "$elemMatch": {
                ...
                if(filter != undefined){
                    ...
                    condition.$and.push(findByBranchCondition);
                    //------------------------------------------------------------------------------------------------//

                    //Add match operation to aggregations:
                    req.query.aggregate.push({ $match: condition });
                } else {
                    //Add match operation to aggregations (Only findByBranch condition | without filters):
                    req.query.aggregate.push({ $match: findByBranchCondition });
                }
                ...
                res.status(422).send({ success: false, message: currentLang.db.validate_error, validate_errors: currentLang.ris.validate.branch_invalid_ObjectId });
            }
        } else {
            //Return the result (HTML Response):
